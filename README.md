...
- **Parliament/Session dropdowns** — Select from the 36th through 45th Parliament with auto-populated session options; bills load automatically
- **Sort & filter controls** — Sort chronologically (newest/oldest) and filter by legislative status (Royal Assent, Committee, Second Reading, etc.) with live counts
- **Full-text search** — Instant client-side filtering by bill number, title, or sponsor name
- **Persistent state** — Navigate between Dashboard, Chat, and Guide tabs without losing your place or conversation history
...
| **LLM** | Google Gemini 2.5 Flash | Response generation with low-temperature precision |
...
### 1. Browse Bills
Select a Parliament and Session from the dropdowns. Bills load automatically. Use the search bar, sort control (Newest / Oldest), and status filter to find what you need.

### 2. Chat About a Bill (Bill Mode)
Click any bill card to open the chat. The system auto-ingests the bill text and debate transcripts on your first question — no setup required. Ask anything: "What are the key provisions?", "What criticisms were raised in debate?", "Who sponsored this bill?"

### 3. Chat About Regulations (General Mode)
Click "Chat with Gazette Regulations" on the Dashboard to load regulatory data and enter General Mode. Ask about proposed regulations, cost-benefit analyses, or any cross-cutting topic across all loaded data.
...
3. **Fallback logic** — If filtered search returns nothing, retries without the filter; if still empty, uses LLM general knowledge with a disclaimer
4. **Prompt construction** — Retrieved chunks are formatted with source labels and passed as context to Gemini Flash
5. **Generation** — Gemini generates a response grounded in the context, using `[Source N]` notation for citations
